...
def billing_details(request):
    if request.method == 'POST':
        ...
        create_order = Sale.objects.create(customer_name=cus_name, item=item, quantity=quantity, price=price,
                                           payment_method=payment_method, profile=user)
        create_order.save()
...
def add_item(request,slug):
    ...
    items = Item.objects.all()
    biling_detail = Billing.objects.all()
    return render(request,'transactions/salescreate.html',
                  {'price': price, 'item_name': item_name, 'items': items, 'biling_details': biling_detail})
...
def delete_bill_single(request,product_name_to_delete):
    billing_instances_to_delete = Billing.objects.filter(sale__item__name=product_name_to_delete)
